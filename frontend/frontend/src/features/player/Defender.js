...
import {fetch_stats_defenders} from "../playersPivot/playerPivotSlice";
    ...
    render() {
        ...
        return (
            <Container>
                ...
                <Row  style={mystyle}>
                    <Col className={'col-md-8 col-md-offset-2'}>
                        <Card title={'Compare passing analytics'}>
...
const mapStateToProps = state => {
    const {current_player_id,statistics_defenders} = state.playersPivot;
    //playerinfo
    const {player_info,player_info_loading} = state.defender;
    //duels info
    const {duels_info,duels_analytics} = state.defender;
    const {passes_info,passes_analytics} = state.defender;

    const {matches, rankings} = state.defender;

    const {duels_stats,passes_stats,matches_stats,rankings_stats} = state.defender;

    return {current_player_id,player_info,
        duels_info,duels_analytics,
        passes_info,passes_analytics,player_info_loading,matches, rankings,statistics_defenders,duels_stats,passes_stats,matches_stats,rankings_stats}
};

const mapDispatchToProps = dispatch =>  {
    return{
        fetch_stats_defenders: () => dispatch(fetch_stats_defenders()),
