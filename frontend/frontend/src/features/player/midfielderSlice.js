...
import {midfielders_failure, midfielders_success, loading_midfielders} from "../playersPivot/playerPivotSlice";


const midfielderSlicer = createSlice({
    name: 'midfielder',
        initialState:{
        player: null,
        loading_error: false
    ...
    reducers:{
        ...
        loading_shots_stats(state,action){
            state.duel_stats_loading= true
        },
        loading_shots_stats_success(state, action){
            state.shots_stats = action.payload;
            state.shots_stats_loading =false;
            state.shots_stats_loading_error = false
        } ,
        loading_shots_stats_failure(state,action) {
            state.shots_stats_loading_error = action.payload;
            state.shots_stats_loading = false
        },
...
export const {
    ...
    loading_shots_stats,
    loading_shots_stats_success,
    loading_shots_stats_failure,
...
} = midfielderSlicer.actions;


///PLAYER INFO
export const fetch_player_info = (playerId,position) =>  async (dispatch) => {
    const API_PLAYER_INFO = 'http://35.246.111.237/api/midfielders/playerinfo/' + playerId;
    dispatch(loading_player_info());

    await axios.get(API_PLAYER_INFO)
        .then((player)=>{
...
export const fetch_duels_info = (playerId) => async (dispatch) => {
    const API_DUELS_INFO = 'http://35.246.111.237/api/midfielders/duels/info/' + playerId;
...
export const fetch_duels_analytics = (playerId) => async (dispatch) => {
    const API_DUELS_ANALYTICS = 'http://35.246.111.237/api/midfielders/duels/analytics/' + playerId;
...
export const fetch_passes_info = (playerId) => async (dispatch) => {
    const API_PASSES_INFO = 'http://35.246.111.237/api/midfielders/passes/info/' + playerId;
...
export const fetch_passes_analytics = (playerId) => async (dispatch) => {
    const API_PASSES_ANALYTICS = 'http://35.246.111.237/api/midfielders/passes/analytics/' + playerId;
...
export const fetch_shots_info = (playerId) => async (dispatch) => {
    const API_SHOTS_INFO = 'http://35.246.111.237/api/midfielders/shots/info/' + playerId;
...
export const fetch_shots_analytics = (playerId) => async (dispatch) => {
    const API_SHOTS_ANALYTICS = 'http://35.246.111.237/api/midfielders/shots/analytics/' + playerId;
...
export const fetch_matches = (playerId) => async (dispatch) => {
    const API_MATCHES = 'http://35.246.111.237/api/midfielders/matches/' + playerId;
...
export const fetch_rankings = (playerId) => async (dispatch) => {
    const API_RANKINGS = 'http://35.246.111.237/api/midfielders/rankings/' + playerId;
...
export const fetch_duels_stats = () => (dispatch) => {
    const API_DUELS_STATS = 'http://35.246.111.237/api/midfielders/duels/analytics/stats';
...
export const fetch_passes_stats = () => (dispatch) => {
    const API_PASSES_STATS = 'http://35.246.111.237/api/midfielders/passes/analytics/stats';
...
export const fetch_shots_stats = () => (dispatch) => {
    const API_SHOTS_STATS = 'http://35.246.111.237/api/midfielders/shots/analytics/stats';
    dispatch(loading_shots_stats());
    return new Promise((resolve,reject)=>{
        axios.get(API_SHOTS_STATS)
            .then((player)=>{
                dispatch(loading_shots_stats_success(player.data));
                resolve()
            }).catch((err)=>{
            dispatch(loading_shots_stats_failure());
            reject()
        })
...
export const fetch_rankings_stats = () => (dispatch) => {
    const API_RANKINGS_STATS = 'http://35.246.111.237/api/midfielders/rankings/stats';
...
export const fetch_matches_stats = (playerId) => (dispatch) => {
    const API_MATCHES_STATS = 'http://35.246.111.237/api/midfielders/matches/stats';
...
export default midfielderSlicer.reducer